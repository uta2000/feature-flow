...
Skill(skill: "feature-flow:create-issue", args: "yolo: true. [original args]")
```
...
- **File-based state** — `.feature-flow.yml`, design docs, implementation plans still written
...
**Excluded:**
- Persistent YOLO config in `.feature-flow.yml` (per-invocation only — intentional)
