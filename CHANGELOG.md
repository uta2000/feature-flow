...
### Added
- **Worktree-first lifecycle + design-in-issue + post-merge cleanup (#244)** — Three coupled lifecycle changes that make concurrent `start:` sessions safe and durable: (1) Worktree setup moves to **step 1** in all non-quick scopes (Small enhancement standard + fast-track, Feature, Major feature). Each session now isolates immediately — no shared CWD on the base branch while brainstorming/design/planning runs. Quick fix remains unchanged (no worktree, single-commit path). Provisional slug at step 1 uses `kebab(first-4-content-words(description)) + "-" + sha256(description + iso_date_utc)[:4]` (60-char cap; `-2`/`-3` suffix on directory collision). (2) Design content moves **from `docs/plans/YYYY-MM-DD-*.md` files into the GitHub issue body**, merged between `<!-- feature-flow:design:start -->` / `<!-- feature-flow:design:end -->` markers via `gh issue edit --body-file`. `create-issue` now always runs before `design-document`; both `design-verification` and `writing-plans` read the marker-bounded section from the issue body. Overflow past 65,536 chars falls back to a standalone issue comment with a marker-wrapped reference link in the body. Malformed/duplicated markers fall back to append mode with a warning. The `## Commit Planning Artifacts` inline step is removed entirely — no more writing to `docs/plans/` on the base branch. Historical plan files stay as archive; new `docs/plans/README.md` advisory points readers to linked issues for active designs. (3) New **`feature-flow:cleanup-merged`** skill (idempotent, best-effort, non-fatal) drives post-merge cleanup: `git worktree remove --force` → `rm -rf` fallback → `git branch -D` → `git push origin --delete` (gated on `cleanup.delete_remote_branch`, default `true`) → remove handoff YAML → append to `.feature-flow/handoffs/.log`. Invoked by `merge-prs` after each successful merge AND opportunistically as a pre-flight at every `start:` session kickoff to self-heal from skipped/UI-driven merges and crashed sessions. Handoff state lives at `.feature-flow/handoffs/<pr-number>.yml` in the **base repo** (gitignored via the existing `.feature-flow/` entry), so the file survives worktree removal; it's created as `pending-<slug>.yml` at worktree setup and renamed at the Commit-and-PR step. Handoff step no longer removes the worktree. PR-body `feature-flow-metadata` block schema renamed `design_doc` → `design_issue` (integer) and `conflict-resolution.md` cross-session recovery falls back to `gh issue search` instead of `ls docs/plans/`. Lifecycle context key renamed `design_doc` → `design_issue`; skill-invocation mapping key standardized on `issue` (was `existing_issue`). Dispatcher `.dispatcher-worktrees/` is unchanged (out of scope).
