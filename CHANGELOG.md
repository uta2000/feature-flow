...
### Changed
- **`start:` Wait-for-CI step parallelizes Phase 1 and Phase 2a (#252, PR #259)** — Phase 1 (CI polling) and Phase 2a (bot-review history detection) now fire in parallel at step entry instead of sequentially. Adds an explicit Step Exit / Join Condition section defining when both loops terminate. Diagram updated to plain-text fork-join style matching existing codebase conventions. Saves 2–5 minutes per Feature/Major-feature lifecycle that has both CI and a review bot. Behavior preservation verified: 15-min Phase 2 timeout, max 2 fix-recheck cycles, no second bot-review round after CI fix push — all unchanged.

## [1.37.0] - 2026-04-23

### Added
- **Worktree-first lifecycle + design-in-issue + post-merge cleanup (#244, PR #245)** — Three coupled lifecycle changes that make concurrent `start:` sessions safe and durable. (1) **Worktree-first**: Worktree setup moves to **step 1** in all non-quick scopes (Small enhancement standard + fast-track, Feature, Major feature). Each session isolates immediately — no shared CWD on the base branch while brainstorming/design/planning runs. Quick fix unchanged (AC-10). Slug: `kebab(first-4-content-words(description)) + "-" + sha256(description + iso_date_utc)[:4]`, 60-char cap, `-2`/`-3` collision suffix. (2) **Design-in-issue**: Design content moves from `docs/plans/YYYY-MM-DD-*.md` files into the GitHub issue body, merged between `<!-- feature-flow:design:start -->` / `<!-- feature-flow:design:end -->` markers via `gh issue edit --body-file`. `create-issue` now runs before `design-document`; both `design-verification` and `writing-plans` read the marker-bounded section from the issue body. Overflow past 65,536 chars falls back to a standalone issue comment with a marker-wrapped reference link. Malformed/duplicated markers fall back to append mode with a warning. The `## Commit Planning Artifacts` inline step is removed entirely. Historical `docs/plans/*.md` files stay as archive; new `docs/plans/README.md` advisory points readers to linked issues for active designs. (3) **Deferred cleanup**: New `feature-flow:cleanup-merged` skill (idempotent, best-effort, non-fatal) reclaims worktrees/branches/handoff files after a PR merges. Six actions: `git worktree remove --force` → `rm -rf` fallback → `git branch -D` → `git push origin --delete` (gated on `cleanup.delete_remote_branch`, default `true`) → remove handoff YAML → append to `.feature-flow/handoffs/.log`. Step 1 `cd`s to the base repo via `git rev-parse --show-toplevel` so relative paths work regardless of invocation CWD. Action 4 distinguishes "remote ref does not exist" (treat as success) from auth/network/permission errors (retain handoff for retry). Invoked by `merge-prs` on each successful merge AND opportunistically at every `start:` session kickoff. Handoff state at `.feature-flow/handoffs/<pr-number>.yml` in the base repo (gitignored) — **written once, after `gh pr create` returns the PR number, with the PR number in the filename from the start** (no `pending-<slug>.yml`, no rename — eliminates rename-atomicity failure mode). Schema rename: PR-body metadata `design_doc` → `design_issue` (integer); `conflict-resolution.md` cross-session recovery uses `gh issue search` instead of `ls docs/plans/`. Skill-invocation mapping key standardized on `issue` (was `existing_issue`). `design-document` codex proactive-review block writes a session-local snapshot at `.feature-flow/design/design-snapshot-<issue>.md` as a transitional bridge so `consult-codex`'s existing `design_doc_path` contract still works (proper issue-body integration flagged as follow-up). Dispatcher `.dispatcher-worktrees/` unchanged (out of scope).

## [1.36.0] - 2026-04-17

Bundles five features shipped on 2026-04-17: scope-critique pass (#238), advisor integration (#236), senior developer panel (#239), quick-path triage (#234), and codex consultation (#235).

### Added
- **Scope-critique pass for design-verification (#238)** — New `### Step 4.5: Scope critique` inside `## Process` runs after the existing compatibility check and asks five strategic-shape questions (scope, dependencies, simpler alternatives, observability, config surface) before a design reaches `create-issue`. Catches classes of issues the existing gate chain misses by design: oversized scope, phantom cross-issue dependencies, machinery around unobservable capabilities, config bloat, and misplaced documentation. The Step 4 Consolidation call-out explicitly owns BLOCKER-row promotion into Step 5 `### Blockers` with label `[Scope critique Q<N>]`. `assumptions-only` mode is explicitly excluded (strategic critique is out of scope for the narrow assumption-verification mode). Q2's evidence plumbing prefers `exploration_results.schema/pipeline/ui` first, opening referenced files directly only when the dependency's domain isn't covered. A new reference file `skills/design-verification/references/scope-critique.md` holds the expanded prompt text, per-question red-flag lists, and a 5-item bias-resistance checklist (anchoring, completeness bias, scope-preservation, stopping-at-first-fix, technical-lens tunnel vision). Scope-independence is documented alongside Category 25 in the Verification Depth section. Pure markdown — zero JS, zero config schema changes, zero hook changes.
- **Advisor tool integration (#236)** — Two additive changes complementing the existing codex-consultation feature (#235). (1) Four soft advisor-call hints added to judgment-heavy skills: `design-verification` (before declaring blockers), `verify-acceptance-criteria` (before manual interpretation), and `consult-codex` (two hints — near `mode: stuck` documentation and before the Phase 2 codex invocation). Hints are non-blocking — Claude decides per-invocation whether to call `advisor()`. (2) Onboarding/settings hint system: new `hooks/scripts/advisor-hint.js` SessionStart script (registered as a third command alongside `version-check.js`) displays a one-line nudge when the user is on a Sonnet model without the `advisor-tool-2026-03-01` beta header configured, rate-limited to once per calendar day with a permanent-dismiss escape hatch. New `skills/settings/scripts/check-advisor.js` provides platform-aware settings.json detection (macOS/Linux/XDG_CONFIG_HOME/Windows APPDATA) and is shared between the hook and the new `feature-flow:settings advisor [dismiss]` sub-action. Full onboarding documentation at `docs/advisor.md` with a one-paragraph pointer under "Optional enhancements" in the root README. Two new `.feature-flow.yml` keys: `advisor.enabled` (master kill switch, default `true`) and `hints.advisor.dismissed` (default `false`). Beta header value centralized in `skills/settings/scripts/advisor-headers.js` for single-point updates when Anthropic GAs or renames the feature. Tiered stuck-mode machinery and per-call observability are explicitly deferred until Claude Code surfaces an advisor-invocation signal (advisor runs as a server-side sub-inference with no hook-visible surface today). No new hook event types, no new skill — everything extends existing infrastructure.
- **Senior developer panel review (Phase 1c) (#239)** — A new judgment-review phase fires automatically during code-review **at Major-feature scope only**. It surfaces concerns that rule-based agents miss by design: wrong abstractions, operability risk (missing timeouts, unclear failure modes, unbounded resources), and scope fit (scope creep, incidental complexity, behavior regressions). The panel reviews three lenses — Staff Engineer, SRE, and Product Engineer — and its findings appear in the Phase 5 report under a new `### Senior Panel — Judgment Findings` subsection grouped by persona. Judgment findings are **report-only** — unlike rule findings, they are not auto-patched; the user reviews each and decides whether to address, defer, or reject. Diffs over 1500 changed lines skip Phase 1c entirely (opus context-window protection). At sub-Major scope, Phase 1c does not run and the Phase 5 subsection is omitted. See `skills/start/references/senior-panel.md` for the persona contract and `skills/start/references/senior-panel-fixtures.md` for the guard's test payloads.
- **Quick-path triage with code-aware scope confirmation (#234)** — `start:` Triage section (renamed from "Tool Selection") is now a 3-way decision: **⚡ quick** / feature-flow / GSD. The quick path routes bounded trivial changes (prose edits, non-log string literals, comments) to a bare implement-and-commit flow, skipping brainstorm / design / verify / plan / acceptance criteria / handoff. Quick path is only taken when five ordered gates (0–4) confirm scope via a read-only pass (≤5 Bash/Grep/Read/Glob tool calls): Gate 0 (clean tree), Gate 1 (concrete target), Gate 2 (≤ `max_files` files, default 3), Gate 3 (no exported-declaration overlap), Gate 4 (lexical-region rule: Markdown prose outside fences, non-log-arg string literals, or comments; log-call string args excluded via AST ancestor walk with root-identifier match on log/logger/console/logging). Any gate failure silently falls through to the existing feature-flow/GSD heuristic scoring, unchanged. A post-hook `git diff --numstat` budget check (≤ `max_changed_lines`, default 10, measured after Stop hook to include auto-format) and a hard-assertion escape hatch with Gate-0-safe multi-file atomic rollback guard against misclassification. New `tool_selector.quick_path` config sub-section (`enabled`, `max_confirmation_tool_calls`, `max_files`, `max_changed_lines`) with defaults that leave quick path on out of the box. New `--no-quick` CLI flag disables quick path for one invocation; no `--quick` flag is added. 14 test fixtures under `tests/start/quick_path/` cover all acceptance criteria. Language coverage: Markdown, TypeScript, JavaScript, Python; unsupported languages conservatively fail Gate 4.
...
### Changed
- **Phase 3 commit-message prefix (#239)** — renamed from `fix: apply code review fixes` to `fix: apply rule-based code review fixes`. **Migration:** if any external tooling greps git log for the old prefix (release-notes generators, commit analytics, changelog aggregators, internal dashboards), update the pattern to match the new prefix — or match the shared infix `code review fixes` which appears in both. No internal feature-flow code or reference file greps the old string; this note is for downstream consumers.

## [1.35.0] - 2026-04-08

### Added
- **Feature-flow-metadata block in PR bodies (#229)** — Lifecycle now writes a versioned `feature-flow-metadata` YAML block inside an HTML comment to every PR body (opt-out via `lifecycle.metadata_block.enabled: false`). The block captures lifecycle session context — design doc reference, scope, risk tier, acceptance-criteria verification state, touched risk areas, sibling/dependent PRs, and an append-only remediation log — so that downstream consumers no longer have to reconstruct intent from diffs and PR titles. The lifecycle is the **single writer**; `/merge-prs` is a **read-only consumer** with graceful fallback to diff-based inference when the block is absent, unparseable, unknown-version, or missing required fields. New canonical schema reference at `references/feature-flow-metadata-schema.md` (single source of truth: §Field Reference, §Serialization, §Parsing, §Update Protocol, §Version Compatibility, §Complete Example, §Producer/Consumer Contract). New PR Metadata Block Step in `skills/start/references/inline-steps.md` runs in all modes (YOLO, Express, Interactive) during the Commit and PR phase, with `remediation_log` updates wired into Phase 2c (review-comment fixes) and Phase 3 (CI failure fixes) of "Wait for CI and Address Reviews". `skills/merge-prs/SKILL.md` Step 4a now parses the block at sub-step 4a.0 (existing pre-merge checks renamed to 4a.1), binding `metadata.sibling_prs/depends_on_prs/risk_areas/remediation_log` for use in dependency analysis and CI remediation de-duplication. `skills/merge-prs/references/dependency-analysis.md` adds a §Metadata Block Precedence section before §Step 1 documenting the precedence rule (metadata fields are additive over diff-based inference, never replacing). Five fixture files under `skills/merge-prs/references/fixtures/` exercise happy/minimal/unparseable/absent/unknown-version paths. New `lifecycle.metadata_block.enabled` config field (default `true`) in `.feature-flow.yml` schema.
