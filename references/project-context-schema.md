...
plugin_registry:       # Auto-generated: plugin scan results — do not edit manually
  ...
  content_hashes: {}
  plugins: {}
plugin_overrides:      # Optional: manual role/affinity overrides and exclusions
  some-plugin:
    exclude: true
```
...
Auto-generated map of all discovered plugins and their classified capabilities. Persisted under `plugin_registry` in `.feature-flow.yml`. This block is written and maintained by feature-flow — do not edit manually.

**Auto-generated:** The `start` skill writes this block during Step 0 pre-flight after scanning `~/.claude/plugins/cache/`. Subsequent sessions diff the scan results against this persisted registry to detect additions and removals.

**Registry keys:** Each plugin entry uses the `<marketplace>/<plugin-name>` format to avoid namespace collisions across marketplaces.

**Sub-fields:**

| Field | Type | Description |
|-------|------|-------------|
| `last_scan` | ISO 8601 timestamp string | When the plugin cache was last fully scanned |
| `content_hashes` | map of `plugin_key → string` | SHA-256 hash of each plugin's `plugin.json` content — used as a scan fast path to skip unchanged plugins |
| `plugins` | map of `plugin_key → PluginEntry` | Registry of all known plugins, keyed by `<marketplace>/<plugin-name>` |

**`PluginEntry` fields:**

| Field | Type | Description |
|-------|------|-------------|
| `source` | `PluginSource` | Whether the plugin is a hardcoded base plugin or dynamically discovered |
| `status` | `PluginStatus` | Current installation state of the plugin |
| `roles` | list of `RoleEntry` | Lifecycle roles this plugin fulfils, one entry per matched component |
| `stack_affinity` | `["*"]` or list of `StackIdentifier` | Stacks this plugin applies to; `["*"]` means universal |

**`RoleEntry` fields:**

| Field | Type | Description |
|-------|------|-------------|
| `step` | `LifecycleRole` | Lifecycle step this role matches |
| `component` | string | Component reference in `<plugin-name>:<component-name>` format |
| `type` | `ComponentType` | Whether the component is a `skill` or `agent` |
| `confidence` | `Confidence` | Classification confidence (`high` or `low`) — present on discovered plugins only; omitted for base plugins |

**Format:**

```yaml
plugin_registry:  # auto-generated by feature-flow — do not edit manually
  last_scan: "2026-04-01T12:00:00Z"
  content_hashes:
    superpowers-marketplace/superpowers: "abc123def456..."
    claude-plugins-official/context7: "789xyz..."
  plugins:
...
**When absent:** On first run, if `~/.claude/plugins/cache/` is empty or missing, the registry is initialized with base plugins only (all with `status: missing`). The lifecycle continues using namespace-prefix fallback validation to determine actual availability.

**Committed to git:** Yes — committing the registry enables the team to share plugin discovery state. However, since it is auto-generated, merge conflicts should be resolved by accepting either version and letting the next session rescan.

### `plugin_overrides`

Optional manual overrides for the plugin registry. When present, entries in this map take precedence over auto-classified roles and stack affinity for the named plugin.

**Override precedence:**
1. `plugin_overrides` (highest — explicit user intent)
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `roles` | list of `LifecycleRole` | (auto-classified) | Force-assign specific lifecycle roles to this plugin, overriding keyword classification |
| `stack_affinity` | list of `StackIdentifier` | (auto-inferred) | Override the inferred stack affinity with an explicit list |
| `exclude` | boolean | `false` | When `true`, this plugin is never dispatched regardless of its roles or stack affinity |
...
plugin_overrides:
  my-custom-plugin:
    roles: [code_review, testing]    # force-assign roles (overrides classification)
    stack_affinity: [python]         # override inferred affinity
  python-linter:
    exclude: true                    # never use this plugin
```

**When needed:** Only when auto-classification assigns incorrect roles, infers the wrong stack affinity, or you want to permanently suppress a plugin. Most projects can omit this section.
...
Identifies which lifecycle step a plugin component is dispatched for.

| Value | Used for |
|-------|----------|
| `code_review` | Code review pipeline (Phase 1b in the review step) |
| `security_review` | Security-focused code review agents |
| `testing` | Test generation and coverage analysis |
| `design` | Architecture, schema, and brainstorming assistance |
| `documentation` | API docs, README generation, jsdoc tooling |
...
Identifies a technology stack for `stack_affinity` filtering.

| Value | Technology |
|-------|------------|
| `typescript` | TypeScript |
| `python` | Python (including Django, Flask, FastAPI) |
| `react` | React |
| `next-js` | Next.js |
| `node-js` | Node.js (including Express, Fastify) |
| `go` | Go / Golang |
| `java` | Java (including Spring, Kotlin) |
| `ruby` | Ruby (including Rails) |
| `supabase` | Supabase |
| `vue` | Vue / Nuxt |
| `angular` | Angular |
...
Indicates how the plugin was added to the registry.

| Value | Meaning |
|-------|---------|
| `base` | Hardcoded base plugin with known roles (superpowers, context7, pr-review-toolkit, feature-dev, backend-api-security) |
| `discovered` | Dynamically found in `~/.claude/plugins/cache/` and classified via keyword matching |

### `PluginStatus`

Reflects the installation and session-load state of a plugin.

| Value | Meaning |
|-------|---------|
| `installed` | Plugin is present in cache and loaded in the current Claude session |
| `missing` | Plugin is not present in `~/.claude/plugins/cache/` |
| `installed_not_loaded` | Plugin is in cache but not loaded in the current session — may require a Claude Code restart |

### `ComponentType`

Identifies the kind of Claude Code component a role entry refers to.

| Value | Meaning |
|-------|---------|
| `skill` | A slash-command skill (directory with `SKILL.md`) |
| `agent` | A background agent (`.md` file with YAML frontmatter) |

### `Confidence`

Classification confidence for dynamically discovered plugin roles.

| Value | Meaning |
|-------|---------|
| `high` | Keyword match score ≥ 2 (strong signal from multiple keywords or plugin.json keyword bonus) |
| `low` | Keyword match score = 1 (weak signal — single keyword match) |

> `confidence` is only present on role entries for `source: discovered` plugins. Base plugins omit this field because their roles are hardcoded, not classified.
