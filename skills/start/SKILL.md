...
1. **Read state file** and extract `phase_summaries.plan.return_contract`:
   ```bash
...
```yaml
schema_version: 2                          # integer — schema version for migration safety
...
phase_summaries:
  brainstorm:
    ...
    return_contract: <object|null>         # structured return contract from phase skill; null until phase completes
  design:
    completed: <bool>
    issue_url: <url|null>
    key_decisions: [<string>, ...]
