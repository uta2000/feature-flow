...
| Commit and PR | `superpowers:finishing-a-development-branch` | PR URL; PR body includes `feature-flow-metadata` block (all modes). **Handoff file rename:** After PR creation, rename `.feature-flow/handoffs/pending-<slug>.yml` to `.feature-flow/handoffs/<pr-number>.yml` to associate the handoff state with the actual PR number. |
