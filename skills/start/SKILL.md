...
d = yaml.safe_load(open(f))
d["phase_summaries"][phase]["completed"] = True
...
For phases with non-list fields (`design.issue_url`, `plan.plan_path`, `implementation.tasks_done` / `tasks_total`), pass each as its own env var (`URL=...`, `PATH=...`, `DONE=...`, `TOTAL=...`) and read with `os.environ[<NAME>]`. Use `int(os.environ["DONE"])` for integers. **Use `--git-common-dir | xargs ... && pwd` for `BASE_REPO`, never `--show-toplevel`** — the latter returns the worktree path from inside a worktree.
