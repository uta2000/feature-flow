...
| Commit and PR | `superpowers:finishing-a-development-branch` | PR URL; PR body includes `feature-flow-metadata` block (all modes). **In-progress file swap (delete-before-create):** Immediately after `gh pr create` returns the PR number and **before** writing the numbered handoff file, delete the in-progress file: `rm -f "$(git rev-parse --show-toplevel)/.feature-flow/handoffs/in-progress-${SLUG}.yml"` (where `SLUG` is the slug from session context — see slug derivation in the Worktree setup row). `rm -f` exits cleanly when the file does not exist (e.g., the initial in-progress write failed); do not error. The two file forms are mutually exclusive — only one exists per slug at a time. **Handoff file write:** Immediately after the in-progress file swap above and after `gh pr create` returns the PR number, write `.feature-flow/handoffs/<pr-number>.yml` in the **base repo** (not the worktree — the file must survive worktree removal). Required fields: `schema_version: 1`, `pr_number: <N>`, `branch: <branch>`, `worktree_path: <abs-path>`, `base_branch: <base>`, `scope: <scope>`, `issue_number: <N-or-null>`, `created_at: <iso-utc>`, `slug: <slug>`, `feature_flow_version: <plugin-version>`. The write happens once, in a single step — no intermediate `pending-<slug>.yml` file, no rename. If the write fails, log a warning and continue (the worktree is still valid; the user can retry by running `cleanup-merged` manually later, though cleanup without a handoff requires manual worktree removal). |
