...
# Tags that indicate a "user" message was generated by the system, not typed
# by the human.  Checked against the first 500 chars of text content.
_AUTOMATED_CONTENT_TAGS = (
...
def is_human_input(message):
    """Return True only if *message* represents genuine human keyboard input.

    Claude Code wraps tool results, skill loads, hook outputs, compaction
    summaries, and continuation messages as ``role=user`` messages.  These are
    automated and should not count toward idle-time calculation.

    Heuristic (all must be true):
    1. ``type == "user"``
    2. No tool-result metadata keys (``toolUseResult``, ``sourceToolAssistantUUID``,
       ``sourceToolUseID``)
    3. ``isMeta`` is falsy
    4. ``isCompactSummary`` is falsy
    5. Content is a plain string (not a list containing ``tool_result`` blocks)
    6. Content does not start with known automated tags
    """
    if message.get("type") != "user":
        return False
    if any(k in message for k in ("toolUseResult", "sourceToolAssistantUUID",
                                   "sourceToolUseID")):
        return False
    ...
    content = message.get("content", "")
    # List content with tool_result blocks is automated
    if isinstance(content, list):
        if any(
    ...
    prefix = content[:500]
    return not any(tag in prefix for tag in _AUTOMATED_CONTENT_TAGS)
...
def analyze_session(filepath):
    ...
    # Manual: user ran /compact -> track start ts until Compacted/isCompactSummary
    # Automatic: isCompactSummary without preceding /compact command
    pending_compact_start_ts = None   # ts when /compact command was seen
    compaction_durations = []         # list of {type, duration_seconds, msg_index}
    ...
    for i, m in enumerate(messages):
        ...
        # --- Compact summaries & compaction duration tracking ---
        # Manual compaction: user types /compact, system outputs "Compacted".
        # Message order: /compact command [N] -> "Compacted" stdout [N+1].
        # Automatic compaction: isCompactSummary without preceding /compact.
        content_text = extract_text_content(m)
        if "<command-name>/compact</command-name>" in content_text and msg_ts:
            pending_compact_start_ts = parse_timestamp(msg_ts)

        # "Compacted" stdout = manual compaction completed
        if "Compacted" in content_text and "local-command-stdout" in content_text:
            if pending_compact_start_ts is not None and msg_ts:
                end_ts = parse_timestamp(msg_ts)
                cdur = (end_ts - pending_compact_start_ts).total_seconds()
                compaction_durations.append({
                    "type": "manual",
                    "duration_seconds": round(cdur, 1),
                    "message_index": i,
                })
                pending_compact_start_ts = None

        if m.get("isCompactSummary"):
            compact_count += 1
            # Only track as automatic if there is no /compact command nearby.
            # The manual "Compacted" stdout message may appear after this
            # isCompactSummary in index order, so we can't rely on the
            # compaction_durations list — check the raw messages instead.
            has_nearby_compact_cmd = False
            for j in range(max(0, i - 5), min(len(messages), i + 5)):
                cj = extract_text_content(messages[j])
                if "<command-name>/compact</command-name>" in cj:
                    has_nearby_compact_cmd = True
                    break
            already_tracked = has_nearby_compact_cmd
            if not already_tracked and msg_ts:
                end_ts = parse_timestamp(msg_ts)
                # Find the last assistant message before this compaction.
                # The immediately preceding messages are often system messages
                # at the same timestamp (part of the compaction event itself),
                # so we scan backward for the last true assistant response.
                pre_compact_ts = None
                for j in range(i - 1, max(0, i - 30), -1):
                    mj = messages[j]
                    if mj.get("type") == "assistant" and mj.get("timestamp"):
                        pre_compact_ts = parse_timestamp(mj["timestamp"])
                        break
                if pre_compact_ts:
                    cdur = (end_ts - pre_compact_ts).total_seconds()
                    compaction_durations.append({
                        "type": "automatic",
                        "duration_seconds": round(cdur, 1),
                        "message_index": i,
        ...
        # --- Idle gap detection ---
        # Measure gaps between the last system activity and genuine human
        # input.  "System activity" includes assistant responses, tool
        # results, compaction processing, hook outputs, and any other
        # automated message — anything the system produced without the
        # human typing.  Only the gap from the end of system activity to
        # the next genuine human keystroke counts as idle.
        if not is_human_input(m) and msg_ts:
            last_assistant_ts = parse_timestamp(msg_ts)
        if is_human_input(m) and msg_ts and last_assistant_ts:
    ...
    # --- Compaction (enhanced with phase interpretation + duration tracking) ---
    phases = session.get("phaseBreakdown", [])
    manual_compactions = [c for c in compaction_durations if c["type"] == "manual"]
    auto_compactions = [c for c in compaction_durations if c["type"] == "automatic"]
    manual_total = sum(
        c["duration_seconds"] for c in manual_compactions
        if c["duration_seconds"] is not None
    )
    auto_total = sum(
        c["duration_seconds"] for c in auto_compactions
        if c["duration_seconds"] is not None
    )
    compaction_total = manual_total + auto_total
    compaction_pct = (
        round(compaction_total / wall_clock * 100, 1) if wall_clock else 0
    )
    ...
    report["compaction"] = {
        ...
        "durations": compaction_durations,
        "manual_count": len(manual_compactions),
        "manual_total_seconds": round(manual_total, 1),
        "automatic_count": len(auto_compactions),
        "automatic_total_seconds": round(auto_total, 1),
        "total_compaction_seconds": round(compaction_total, 1),
        "total_compaction_human": str(timedelta(seconds=int(compaction_total))),
        "compaction_pct_of_wall_clock": compaction_pct,
        "note": (
